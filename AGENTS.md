...
- Meetings: LiveKit Cloud (Free Tier)
...
src/
  ...
  components/
    ...
    meetings/       — LiveKitMeeting, MeetingPreJoin
    ai/             — AISidebar
  lib/
...
- [x] Prompt 19: LiveKit Migration (2026-04-21) — Full migration from Jitsi to LiveKit. Replaced `jitsiRoomId` with `liveKitRoomId` in DB schema. Created `LiveKitMeeting` component using `@livekit/react` and `livekit-client`. Updated token generation in `src/lib/livekit.ts` using `livekit-server-sdk` v2. Cleaned up all Jitsi legacy source and dependencies. Created `livekit-setup-guide.md` and `LIVEKIT_MIGRATION_PROGRESS.md`.
...
### LiveKit Integration
- **CRM embedding**: `src/components/meetings/LiveKitMeeting.tsx` (`@livekit/react`), `MeetingPreJoin.tsx`, project join route `projects/[id]/meetings/join/[meetingId]`.
- **Token Generation**: `src/lib/livekit.ts` uses `AccessToken` from `livekit-server-sdk` with 4h TTL.
- **Database**: `Meeting` model uses `liveKitRoomId` instead of `jitsiRoomId`.

## Change Log
- 2026-04-21 — **LiveKit Migration** · Replaced Jitsi with LiveKit. Removed `jitsi-source/`, `src/lib/jitsi.ts`, `src/components/meetings/JitsiMeeting.tsx`. Updated `package.json` and `prisma/schema.prisma`.
- 2026-04-19 — **Jitsi UX + jitsi-source Windows fix** · (Legacy) `jitsi-source/scripts/android-autolinking.js`, `sync-jitsi-web.sh`.
- 2026-04-17 — **Phase 0.1** · `changes.md` rewritten as living progress log with phase map.
- 2026-04-17 — **Phase 0.2** · Manager ≡ Admin parity completed.
- 2026-04-17 — **Phase 0.3** · Badge UI polish.
- 2026-04-17 — **Phase 0.4** · Public CV locked to PDF.
- 2026-04-17 — **Phase 0.5** · Jitsi JWT fix (Legacy).
